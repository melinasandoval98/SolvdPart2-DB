...
public class ComputerDAO implements IComputerDAO {
    ...
    private final String GET_BY_C_F_R_ID_QUERY = "model, year, data_storage_device_capacity FROM Computers c JOIN Computers_For_Repair cfr ON c.id = cfr.computer_id WHERE cfr.id = ?";
...
//			}
    ...
    @Override
    public Computer getComputerByComputerForRepairID(long computerForRepairID) {
        PreparedStatement prepStat = null;
        ResultSet resultSet = null;
        Computer computer = null;
        try {
            prepStat = connection.prepareStatement(GET_BY_C_F_R_ID_QUERY);
            prepStat.setLong(1, computerForRepairID);
            resultSet = prepStat.executeQuery();
            if (resultSet.next()) {
                computer = createComputer(resultSet);
            } else {
