...
public class ProcessorDAO implements IProcessorDAO {
    ...
    private final String GET_BY_EMPLOYEE_ID_QUERY = "SELECT * FROM Processors p JOIN Computers c ON p.id = c.user_id WHERE c.id = ?";
    ...
    @Override
    public Processors getProcessorByComputerID(long id) {
        PreparedStatement prepStat = null;
        ResultSet resultSet = null;
        Processors processor = null;
        try {
